...
- При получении нового маршрута: проверяет, находится ли на первом ребре нового маршрута
- Если да → сворачивает на новый маршрут
- Если нет (проехал поворот) → продолжает старый маршрут до следующего пересечения
...
- `end_point`: Конечная точка (lat, lon)
