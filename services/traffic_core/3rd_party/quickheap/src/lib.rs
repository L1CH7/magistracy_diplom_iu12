...
#[unsafe(no_mangle)]
pub extern "C" fn simd_quickheap_create() -> *mut SimdQuickHeap<u64> {
    let heap = SimdQuickHeap::<u64>::default();
    Box::into_raw(Box::new(heap))
}

#[unsafe(no_mangle)]
pub extern "C" fn simd_quickheap_destroy(heap: *mut SimdQuickHeap<u64>) {
    if !heap.is_null() {
        unsafe {
...
#[unsafe(no_mangle)]
pub extern "C" fn simd_quickheap_push(heap: *mut SimdQuickHeap<u64>, val: u64) {
    let heap = unsafe { &mut *heap };
    heap.push(val);
}

#[unsafe(no_mangle)]
pub extern "C" fn simd_quickheap_pop(heap: *mut SimdQuickHeap<u64>, out_val: *mut u64) -> bool {
    let heap = unsafe { &mut *heap };
...
#[unsafe(no_mangle)]
pub extern "C" fn simd_quickheap_clear(heap: *mut SimdQuickHeap<u64>) {
    let heap = unsafe { &mut *heap };
    *heap = SimdQuickHeap::<u64>::default();
}

#[unsafe(no_mangle)]
pub extern "C" fn simd_quickheap_size(heap: *const SimdQuickHeap<u64>) -> usize {
    let heap = unsafe { &*heap };
    heap.size
}

#[unsafe(no_mangle)]
pub extern "C" fn simd_quickheap_empty(heap: *const SimdQuickHeap<u64>) -> bool {
    let heap = unsafe { &*heap };
    heap.size == 0
}
