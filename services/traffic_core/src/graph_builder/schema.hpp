...
CREATE INDEX IF NOT EXISTS idx_ecm_old_id ON graphs.edge_candidates_merged(old_id);
ANALYZE graphs.edge_candidates_merged;
)";
...
SELECT ST_SnapToGrid(geom, 0.00001) FROM (
    SELECT ST_StartPoint(geom) AS geom FROM graphs.edge_candidates_merged
    UNION
    SELECT ST_EndPoint(geom)   AS geom FROM graphs.edge_candidates_merged
) AS pts
...
ALTER TABLE graphs.edge_candidates_merged ADD COLUMN IF NOT EXISTS source BIGINT;
ALTER TABLE graphs.edge_candidates_merged ADD COLUMN IF NOT EXISTS target BIGINT;
)";

constexpr std::string_view TOPOLOGY_UPDATE_SOURCES_SQL = R"(
WITH starts AS (
    SELECT e.id AS edge_id, v.id AS node_id
    FROM graphs.edge_candidates_merged e
    JOIN edge_candidates_merged_vertices_pgr v
      ON ST_SnapToGrid(ST_StartPoint(e.geom), 0.00001) = v.the_geom
)
UPDATE graphs.edge_candidates_merged e
...
CREATE TABLE graphs.eb_edges (
    ...
    turn_angle    FLOAT   NOT NULL DEFAULT 0,   -- угол поворота (градусы)
...
normalized AS (
    SELECT *,
        CASE
            WHEN raw_angle IS NULL THEN 0.0
...
INSERT INTO graphs.eb_edges
    (from_eb_node, to_eb_node, junction_node, turn_angle, turn_type, maneuver_time)
SELECT
...
WITH raw_maneuvers AS (
    SELECT
        ...
        f.exit_node AS junction,
        DEGREES(
            ST_Azimuth(ST_StartPoint(t.geom), ST_PointN(t.geom, 2)) -
