...
/**
 * @brief Подсистема противодействия заторам и дедлокам (Deadlock & Queue
 * Mitigation System). Инкапсулирует обработку виртуального буфера SUMO и
 * игрового деспавна Cities Skylines.
 */
...
public:
  ...
  /**
   * @brief Обработка агентов в виртуальном буфере SUMO и очереди ожидания
   * заторов.
  ...
  void Process(float dt, const PhysicsContext &ctx) noexcept {
    ...
    while (vb_idx < pool_.virtual_buffer_queue.size()) {
      ...
      if (ctx.edge_attributes && ctx.live_volumes) {
        ...
        float v_bpr = (live_vol > vis_cap)
                          ? v_free * (C_vis / static_cast<float>(live_vol))
                          : v_free;
      ...
      if (pool_.pos_meters[agent_idx] >= edge_len) {
        ...
        if (next_idx < route.size()) {
          ...
          float load_ratio_next = static_cast<float>(load_next + 1) /
                                  static_cast<float>(jam_cap_next);

          // Проверяем по стандарту SUMO MESO: дорога рассосалась ниже порога
          // затора (meso_jam_threshold_pct)?
          if (load_ratio_next <= ctx.meso_jam_threshold_pct) {
            // Ребро рассосалось! Агент возвращается из виртуального буфера на
            // физическое ребро
            ...
            pool_.virtual_buffer_queue[vb_idx] =
                pool_.virtual_buffer_queue.back();
            pool_.virtual_buffer_queue.pop_back();
            continue;
          } else {
            // Физическое ребро забито, но агент НАХОДИТСЯ В ВИРТУАЛЬНОМ БУФЕРЕ
            // SUMO! Агент переходит на следующее ребро в виртуальном режиме
            // (1.3 м/с)
            pool_.route_progress_idx[agent_idx] = next_idx;
            pool_.current_edge[agent_idx] = next_edge;
            pool_.pos_meters[agent_idx] = 0.0f;
            ...
            float next_len =
                (ctx.edge_lengths_m) ? ctx.edge_lengths_m[next_edge] : 100.0f;
            if (next_len < 1.0f)
              next_len = 1.0f;
    ...
    while (sb_idx < pool_.spillback_wait_queue.size()) {
      ...
      uint32_t wait_time =
          ctx.current_time_sec - pool_.spillback_start_time_sec[agent_idx];
      if (static_cast<float>(wait_time) >= ctx.time_to_teleport_sec) {
        ...
        if (pool_.in_queue[agent_idx]) {
          ...
          if (ctx.queue_volumes) {
            uint32_t q =
                ctx.queue_volumes[curr_edge].load(std::memory_order_relaxed);
            if (q > 0)
              ctx.queue_volumes[curr_edge].fetch_sub(1,
                                                     std::memory_order_relaxed);
          }
        }

        if (ctx.live_volumes && ctx.live_volumes[curr_edge] > 0) {
          ctx.live_volumes[curr_edge]--; // Освобождаем место, разрывая
                                         // замкнутое кольцо
        }
