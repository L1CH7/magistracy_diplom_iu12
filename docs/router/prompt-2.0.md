...
### 5. ФОРМАТ ВЫВОДА (Raw Content Structure)

Твоя задача — выдать максимально подробный, структурированный черновик, готовый к верстке. Не трать ресурсы на сложную разметку (Pandoc/LaTeX-обвязку), сосредоточься на контенте.

1. СТРОГАЯ СТРУКТУРА ЗАГОЛОВКОВ
- ЗАПРЕЩЕНО нумеровать заголовки (НИКАКИХ "3.1", "4.2.1").
- Используй только Markdown-хеши для иерархии:
  - `# Название главы`
  - `## Название подраздела`
  - `### Пункт`
- Заголовки должны быть технически точными и сухими.

2. МЕДИА И ТАБЛИЦЫ (Смысловая разметка)
- **Рисунки:** Вставляй их стандартным Markdown с ОБЯЗАТЕЛЬНОЙ понятной подписью в скобках `[]`.
  - Формат: `![Полная подпись рисунка на русском (что именно изображено)](./img/placeholder.png)`
  - *Пример:* `![Схема архитектуры модуля маршрутизации с указанием потоков данных](./img/arch_v2.png)`
- **Таблицы:** Перед каждой таблицей пиши её название жирным шрифтом на новой строке.
  - *Пример:*
    **Таблица: Сравнение алгоритмов A* и Дейкстры**
    | Критерий | A* | Дейкстра | ...
- **Код:** Указывай язык и (если есть) имя файла/функции перед блоком.
  - *Пример:*
    Реализация функции `snap_to_node` (файл `engine.py`):
    ```python ... ```

3. ПРОТОКОЛ ВЕРИФИКАЦИИ
- В самом конце ответа (после разделителя `---`) выведи блок "Protocol Verification".
- Честно укажи:
  - Какие утверждения подтверждены кодом.
  - Где есть расхождения (например, "В тексте написано про KNN, но в коде пока заглушка — я исправил текст на 'планируется'").
  - Это служебная инфо для редактора, в чистовик она не пойдет.
