...
<!-- Эта шапка не обязательна, ее можно чисто вынести в config.yaml -->
...
<!-- Так выглядит невидимый комментарий или ToDo -->
...
~~~ {.c #lst:code1 caption="Пример кода"}
double al = 1;
...
При этом caption автоматом с помощью pandoc подставит название и номер листинга. Листинг 1 - Название. Формат будет определяться в config.yaml для pandoc на каждый структурный элемент.
...
``` {.bash #lst:code2 caption="Пример команды"}
...
Для отображения диаграмм пришлось бороться особым образом с наложением фильтров, потому обрамляем mermaid [-@fig:mermaid-id] в div от pandoc.
...
Последняя строка это название диаграммы mermaid
:::
