"""
Navigation MAS — Main Window (modular version of gui_old.py).
This is the complete modular refactor, extracting ALL logic from gui_old.py.
"""
import sys
import os
import threading
...
from handlers.zoom_api_handler import ZoomAPIHandler


class NavigationGUI(QMainWindow):
    """Main window: fullscreen map + collapsible left sidebar.
    
    This is the complete modular version of gui_old.py, preserving ALL logic and style.
    """
    ...
    def setup_ui(self) -> None:
        ...
        Complete extraction from gui_old.py setup_ui method.
        """
        central = QWidget()
        ...
        self.scale_ruler.setVisible(False)
        
        self.scale_ruler_line = QFrame()
        self.scale_ruler_line.setParent(self.map_frame)
        self.scale_ruler_line.setVisible(False)
        
        # Track zoom bounds for slider
        self.current_zoom = 12.0
        self.initial_zoom = 12.0
        self.zoom_min = 0.2 * 12.0
        self.zoom_max = 5.0 * 12.0
    ...
    def _show_map_context_menu(self, lon: float, lat: float) -> None:
        ...
        action_from.triggered.connect(lambda: self._set_point("from", lon, lat))
        action_to.triggered.connect(lambda: self._set_point("to", lon, lat))
        if action_via:
            action_via.triggered.connect(lambda: self._set_point("via", lon, lat))
            ...
            if pos and isinstance(pos, dict):
                ...
                if lon is not None and lat is not None:
                    ...
                    if last_pos_r != (lon_r, lat_r):
                        ...
                        QTimer.singleShot(10, lambda: self._show_map_context_menu(lon, lat))
                        self.web_view.page().runJavaScript("window.lastContextMenuPos = null;")
    ...
    def _set_point(self, point_type: str, lon: float, lat: float) -> None:
        ...
        if point_type == "from":
            color = self.color_palette[self._color_idx % len(self.color_palette)]
            self._color_idx += 1
            
            if not self.points:
                self.points.append({"lon": lon, "lat": lat, "color": color})
            else:
                self.points[0] = {"lon": lon, "lat": lat, "color": color}
            ...
            if len(self.points) == 0:
                ...
                self.points[-1] = {"lon": lon, "lat": lat, "color": color}
            ...
            color = self.color_palette[self._color_idx % len(self.color_palette)]
            self._color_idx += 1
            self.points.insert(-1, {"lon": lon, "lat": lat, "color": color})
    ...
    def _update_map_markers(self) -> None:
        ...
        for idx, point in enumerate(self.points):
            ...
            code = (
                f"window.app && window.app.addMarker({{"
                f"lon: {lon}, lat: {lat}}}, '{marker_color}');"
            )
            self._js(code)
    ...
    def _update_points_list(self) -> None:
        ...
        self.points_container.layout().insertWidget(0, self.points_list_widget)
    ...
    def _create_sandwich_tile(
        ...
        type_label.setStyleSheet("font-size: 11px; color: #374151; min-width: 24px;")
        layout.addWidget(type_label)
        
        coord_text = f"{point['lon']:.2f}, {point['lat']:.2f}"
        coord_label = QLabel(coord_text)
        coord_label.setStyleSheet("font-family: monospace; font-size: 10px; color: #6b7280;")
    ...
    def load_graph(self) -> None:
        ...
        def worker():
            ...
            def apply():
                ...
                code = f"window.app && window.app.setGraphGeoJSON({json.dumps(graph_geojson)});"
                self._js(code)
    ...
    def get_route(self) -> None:
        ...
        k_val = int(self.k_entry.text()) if self.k_entry.text().isdigit() else 1
        req = {
        ...
        def worker():
            try:
                resp = requests.post(f"{self.server_url}/route", json=req, timeout=20)
            ...
            def apply():
                if not ok:
                    self.status_text.append(f"Failed to get route: {data['error']}")
                else:
                    ...
                    for i, r in enumerate(self.routes):
                        self.route_text.append(
                            f"Route {i+1}: {len(r['nodes'])} nodes, "
                            f"{r['total_distance']:.2f}m, {r['estimated_time']:.2f}s"
                        )
                    ...
                    code = f"window.app && window.app.setRoutes({json.dumps(ml_routes)});"
                    self._js(code)
    ...
    def start_agent(self) -> None:
        ...
        try:
            ...
            resp = requests.post(f"{self.server_url}/agent/start", json=req, timeout=10)
            if resp.status_code == 200:
                state = resp.json()
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if self.prev_pos and self.prev_time:
                        lat_diff = state["position"]["lat"] - self.prev_pos["lat"]
                        lon_diff = state["position"]["lon"] - self.prev_pos["lon"]
                    ...
                    self.status_text.clear()
                    self.status_text.append(
                        f"Agent {state['agent_id']} at index {state['index']}, "
                        f"pos: {state['position']}, done: {state['done']}"
                    )
                    pos = state["position"]
                    self._last_agent_pos = pos
                    agent_obj = json.dumps({"lat": pos["lat"], "lon": pos["lon"]})
                    code = f"window.app && window.app.updateAgent({agent_obj});"
                    self._js(code)
                    if state["done"]:
                        break
                else:
    ...
    def _update_bounds_label(self) -> None:
        ...
        def _apply_bounds(result):
            ...
            try:
                ...
                fmt = f"[{bbox[0]:.5f}, {bbox[1]:.5f}, {bbox[2]:.5f}, {bbox[3]:.5f}]"
    ...
    def _update_zoom_slider_from_map(self, zoom_level: float) -> None:
        """Update zoom slider to match current map zoom."""
        try:
            zoom_min = 0
            zoom_max = 19
            slider_pos = ((zoom_level - zoom_min) / (zoom_max - zoom_min)) ** (1.0 / 1.3)
            slider_value = int(max(0, min(100, slider_pos * 100)))
    ...
    def _update_scale_label(self) -> None:
        ...
        def _apply_scale(result):
            if result is not None:
                try:
                    ...
                    lat = 55.7558
                    px_per_meter = (
                        256 * (2 ** zoom) * math.cos(math.radians(lat)) / 40075000
                    )
                    ...
                    best_scale = 1
                    best_diff = float('inf')
                    target_center = (target_pixels_min + target_pixels_max) / 2
        ...
        js = "(function(){ if(!window.map) return null; return window.map.getZoom(); })();"
    ...
    def _update_overlay_positions(self) -> None:
        ...
        if self.sidebar_visible:
            ...
            self.sidebar.resize(self.sidebar.width(), map_h - 16)
            self.sidebar.show()
    ...
    def on_map_loaded(self, ok: bool) -> None:
        ...
        if not self._bounds_timer:
            ...
            self._bounds_timer.timeout.connect(self._update_scale_label)
            self._bounds_timer.start(500)
        if self._graph_geojson is not None:
            code = f"window.app && window.app.setGraphGeoJSON({json.dumps(self._graph_geojson)});"
            self._js(code)
        if self._routes_ml is not None:
            code = f"window.app && window.app.setRoutes({json.dumps(self._routes_ml)});"
            self._js(code)
            self._js("window.app && window.app.fitToRoutes();")
