-- Migration 003: Create graph tables for simulation
...
CREATE INDEX IF NOT EXISTS idx_nodes_osm_id ON nodes(osm_node_id);
CREATE INDEX IF NOT EXISTS idx_nodes_lat_lon ON nodes(lat, lon);

COMMENT ON TABLE nodes IS 'Graph nodes (intersections, endpoints)';
COMMENT ON COLUMN nodes.osm_node_id IS 'Original OSM node ID';
COMMENT ON COLUMN nodes.geometry IS 'PostGIS point geometry for spatial queries';
...
CREATE TABLE IF NOT EXISTS edges (
    ...
    start_node_id INT NOT NULL REFERENCES nodes(id) ON DELETE CASCADE,
    end_node_id INT NOT NULL REFERENCES nodes(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_edges_osm_way ON edges(osm_way_id);
...
CREATE INDEX IF NOT EXISTS idx_edges_highway_type ON edges(highway_type);
CREATE INDEX IF NOT EXISTS idx_edges_current_load ON edges(current_load);

COMMENT ON TABLE edges IS 'Graph edges (road segments with uniform characteristics)';
COMMENT ON COLUMN edges.osm_way_id IS 'Original OSM way ID (for deduplication)';
COMMENT ON COLUMN edges.capacity IS 'Max agents on edge = (length/5) × lanes';
COMMENT ON COLUMN edges.base_travel_time_sec IS 'Travel time at speed_limit with no congestion';
COMMENT ON COLUMN edges.current_load IS 'Current number of agents on this edge (updated by simulator)';
COMMENT ON COLUMN edges.effective_speed_kmh IS 'Current effective speed considering congestion';
COMMENT ON COLUMN edges.osm_tags IS 'Full OSM tags for future use (turn:lanes, maxspeed:conditional, etc)';
...
BEGIN
    ...
    SELECT 
        ...
        (SELECT SUM(length_m) / 1000.0 FROM edges),
        (SELECT AVG(length_m) FROM edges),
        (SELECT AVG(lanes) FROM edges),
        (SELECT SUM(capacity) FROM edges),
...
-- GRANT SELECT, INSERT, UPDATE ON nodes, edges TO diplom;
